...
public class WorldGenGarden
{
    ...
    public void decorateEvent(Decorate event)
    {
        ...
        if (worldIn.provider.getDimension() == 0 && event.getType() == Decorate.EventType.PUMPKIN)
        {
            ...
            if (!biome.canRain() || biome.isSnowyBiome() || biome.getBaseHeight() > 0.4F || biome.decorator.flowersPerChunk < 1 || biome.topBlock.getMaterial() != Material.GRASS || biome instanceof BiomeOcean || rand.nextDouble() > biome.getDefaultTemperature() || rand.nextInt(CuisineConfig.GENERAL.cropsGenRate) > 0)
            {
                return;
            }
