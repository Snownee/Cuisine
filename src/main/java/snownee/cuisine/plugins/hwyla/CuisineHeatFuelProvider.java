...
final class CuisineHeatFuelProvider implements IWailaDataProvider
{
    ...
    {
        if (accessor.getPlayer().isCreative() && accessor.getTileEntity() instanceof IHeatable && ((IHeatable) accessor.getTileEntity()).getHeatHandler() instanceof IFuelHandler)
        {
    ...
    {
        if (te instanceof IHeatable && ((IHeatable) te).getHeatHandler() instanceof IFuelHandler)
        {
            IHeatable heatable = (IHeatable) te;
            tag.setFloat("heat", heatable.getHeatHandler().getHeat());
            tag.setFloat("burnTime", ((IFuelHandler) heatable.getHeatHandler()).getBurnTime());
